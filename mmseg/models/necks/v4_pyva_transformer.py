"""
PYVA (Perspective to View Aggregation) Transformer v4 实现

本模块实现了基于PYVA的视图变换transformer，用于将透视视图（perspective view）
的特征转换为鸟瞰图（Bird's Eye View, BEV）特征。

主要组件：
- CrossViewTransformer: 跨视图注意力机制
- TransformModule: 视图变换模块
- v4_Pyva_transformer: 主要的PYVA transformer实现
"""
import torch
...
def feature_selection(input, dim, index):
    ...
    Args:
        input (Tensor): 输入特征tensor
        dim (int): 选择特征的维度
        index (Tensor): 索引tensor，指定要选择的特征
...
class CrossViewTransformer(nn.Module):
    ...
    该模块通过注意力机制实现透视视图和BEV视图之间的特征对齐和融合。
    使用Query-Key-Value注意力机制来找到透视视图和BEV视图之间的对应关系。
    """
    def __init__(self, in_dim=128):
        super(CrossViewTransformer, self).__init__()
        # Query, Key, Value的投影层，用于计算注意力
        self.query_conv = nn.Conv2d(in_channels=in_dim, out_channels=in_dim // 8, kernel_size=1)
        self.key_conv = nn.Conv2d(in_channels=in_dim, out_channels=in_dim // 8, kernel_size=1)
        self.value_conv = nn.Conv2d(in_channels=in_dim, out_channels=in_dim, kernel_size=1)
        # 融合层，将cross-view特征和transformed特征融合
    ...
    def forward(self, front_x, cross_x, front_x_hat):
        ...
        Args:
            front_x: 透视视图特征 (front view features)
            cross_x: 跨视图特征 (cross-view features)，通常是BEV特征
            front_x_hat: 变换后的透视视图特征 (transformed front view features)
        ...
        # 计算注意力分数 (energy)，找到最匹配的位置
        energy = torch.bmm(proj_key, proj_query)  # transpose check
        front_star, front_star_arg = torch.max(energy, dim=1)  # 找到最大注意力分数和对应位置
        ...
        front_res = front_res * S  # 加权融合
        output = cross_x + front_res  # 残差连接
...
class v4_Pyva_transformer(nn.Module):
    ...
    该模块实现了从透视视图到BEV视图的变换。
    主要流程：
    1. 使用backbone提取透视视图特征
    2. 通过TransformModule进行视图变换
    3. 使用CrossViewTransformer进行跨视图特征融合
    4. 上采样并转换为BEV特征表示
    
    Args:
        size (tuple): 输出特征图尺寸，如 (32, 32)
        back (str): backbone类型，'swin'表示Swin Transformer
        use_fpn (bool): 是否使用FPN
        use_heavy (bool): 是否使用heavy版本的TransformModule
    """
    def __init__(self,size,back = 'swin',use_fpn=False,use_heavy=False):
        super(v4_Pyva_transformer,self).__init__()
        self.size = size  # 中间特征尺寸
        self.back = back  # backbone类型
    ...
    def forward(self,x,calib):
        """
        前向传播：将透视视图特征转换为BEV特征
        
        Args:
            x: backbone提取的特征列表，最后一个元素为主要特征
            calib: 相机标定参数（在此实现中未直接使用，但保留接口兼容性）
            
        Returns:
            feature_final: 最终的BEV特征，shape为 (bs, 64, 98, 100)
            x: 中间特征
            retransform_feature: 重变换后的特征
            transform_feature: 变换后的特征
        """
        x = x[-1]  # 取backbone输出的最后一个特征图，如果使用resnet: bs,2048, 38, 50
        # 如果使用swin作为backbone, x.shape = [bs,768,32,32]
        x = self.interpolate(x) # 插值到统一尺寸: bs,768, 32, 32
        x = self.conv1(x)  # 降维: bs,512, 32, 32
        x = self.pool(x)   # 下采样: bs,512, 16, 16
        x = self.conv2(x)  # 进一步降维: bs,128, 16, 16
        x = self.pool(x)   # 下采样: bs,128, 8, 8
        B,C,H,W = x.shape
        # 视图变换：从透视视图变换到BEV视图
