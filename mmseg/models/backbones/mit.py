...
# 尝试导入normal_init和trunc_normal_init（兼容MMCV 1.x和2.x）
try:
    from mmcv.cnn import normal_init, trunc_normal_init
except ImportError:
    try:
        from mmcv.runner import normal_init, trunc_normal_init
    except ImportError:
        try:
            from mmengine.model import normal_init, trunc_normal_init
        except ImportError:
            # 如果都失败，提供简化实现
            import torch.nn as nn
            import math
            ...
            def trunc_normal_init(module, mean=0, std=1, a=-2, b=2, bias=0):
                """截断正态分布初始化"""
                if hasattr(module, 'weight') and module.weight is not None:
                    # 使用torch的truncated normal初始化
                    with torch.no_grad():
                        nn.init.normal_(module.weight, mean, std)
                        # 简单实现：截断到[a*std, b*std]范围
                        weight = module.weight
                        weight.clamp_(a*std + mean, b*std + mean)
                if hasattr(module, 'bias') and module.bias is not None:
                    nn.init.constant_(module.bias, bias)
# 尝试导入constant_init（兼容MMCV 1.x和2.x）
try:
    from mmcv.cnn import constant_init
except ImportError:
    try:
        from mmcv.runner import constant_init
    except ImportError:
        try:
...
# 尝试从 mmcv.runner 导入 _load_checkpoint（mmcv 1.x），如果失败则尝试其他位置
try:
...
class EfficientMultiheadAttention(MultiheadAttention):
    ...
    def __init__(self,
        ...
        if sr_ratio > 1:
            ...
            # The ret[1] of build-norm_layer is the real norm layer
