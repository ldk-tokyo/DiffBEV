...
import mmcv
import numpy as np
# 尝试导入deprecated_api_warning和is_tuple_of（兼容MMCV 1.x和2.x）
try:
    from mmcv.utils import deprecated_api_warning, is_tuple_of, is_list_of
except ImportError:
    try:
        from mmengine.utils import deprecated_api_warning, is_tuple_of, is_list_of
    except ImportError:
        # 如果都失败，提供简化实现
        import warnings
        ...
        def is_tuple_of(seq, type_or_types):
            """检查序列是否为指定类型的元组"""
        ...
        def is_list_of(seq, type_or_types):
            """检查序列是否为指定类型的列表"""
            if not isinstance(seq, list):
                return False
            return all(isinstance(x, type_or_types) for x in seq)

# 为mmcv添加is_list_of的兼容实现
...
class RandomFlipKitti(object):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            results['img'] = mmcv.imflip(
                results['img'], direction=results['flip_direction'])
            # seg shape: 2x196x200,numpy array
            # flip segs
            for key in results.get('seg_fields', []):
                # use copy() to make numpy stride positive
                results[key]= results[key].astype('uint8')
...
# we should add scale when random crop in gt_semantic_seg
@PIPELINES.register_module()
...
class RandomRotate(object):
    ...
    def __call__(self, results):
        ...
        if rotate:
            # rotate image  image.shape: 1024x1024x3,numpy array,results['img'].dtype: uint8
...
class PhotoMetricDistortion(object):
    ...
    def __call__(self, results):
        ...
        """
        # results['img'] is numpy array, results['img'].shape: [600,800,3]
        img = results['img']
        # cv2.imwrite("/mnt/cfs/algorithm/jiayu.zou/my_mmsegmentation/vis_test/image.jpg",results['img'])
        # print('Finish saving input image!')
        # gt = cv2.imread(os.path.join(results['seg_prefix'],results['ann_info']['seg_map']))
        # gt shape: 196 x 200 x 3
        # cv2.imwrite("/mnt/cfs/algorithm/jiayu.zou/my_mmsegmentation/vis_test/image_gt.png",gt)
